  ...
  perSystem = {pkgs, ...}: {
    checks.swag-unknown-host-404 =
      pkgs.runCommand "swag-unknown-host-404" {
        ...
        check "error-page-js-mime" grep -qF 'application/javascript js;' "$listen"
        absent "error-page-no-duplicate-js" grep -qF 'text/javascript js;' "$listen"
        absent "default-conf-no-sample-version" grep -qF '## Version' "$conf"
